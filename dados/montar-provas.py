...
# O time cap das provas por tempo, em segundos. Quem nao finalizar dentro dele
# entra com as repeticoes que faltaram, e o resultado aparece como "15:10".
CAPS = {
    'Elite':         {1: 15*60, 3: 12*60, 6: 4*60},
    'RX':            {1: 15*60, 3: 12*60, 6: 4*60},
    'Intermediário': {1: 15*60, 3: 12*60, 6: 4*60},
    'Scaled':        {1: 15*60, 3: 12*60, 6: 6*60},
}
...
'Elite': [
 (1, 'PROVA 1', 'tempo',
  ...
  'Obrigatório step down.\n'
  'Não finalizou no cap: deixe o tempo vazio e lance quantas repetições faltaram.'),
 (2, 'PROVA 2', 'reps',
 ...
 (6, 'PROVA 6', 'tempo',
  ...
  'Não finalizou no cap: deixe o tempo vazio e lance quantas repetições faltaram.'),
 (7, 'PROVA 7', 'carga',
  'EMOM 6′ — ímpar: clean & jerk / par: snatch\n\n'
  'Não pode diminuir o peso da barra.\n'
  'Resultado: soma das 6 cargas (3 clean & jerk + 3 snatch).\n'
  'Se não fizer o clean & jerk, o snatch seguinte não é validado.\n'
  'Se errar o snatch, o clean & jerk anterior não é validado.\n\n'
...
'Intermediário': [
 ...
 (4, 'PROVA 4', 'reps',
  ...
  'Resultado: total de repetições nos 3 rounds.'),
 (5, 'PROVA 5', 'reps',
  '1 ROUND 3′ ON — max rounds DT\n'
  'DT = 12 deadlift + 9 hang clean + 6 STOH\n\n'
  'Carga da barra 35/50 kg (mulheres/homens)\n'
...
'Scaled': [
 ...
 (3, 'PROVA 3', 'tempo',
  'FOR TIME 12′\n'
  'Buy in 800 m (mulheres) / 1000 m (homens) de remo\n'
  '9/7 bike\n9/7 thruster\napós 9+7 = 5 shuttle run\n'
  '9/7 bike\n9/7 hang snatch\napós 9+7 = 5 shuttle run\n'
  '9/7 bike\n9/7 STOH\napós 9+7 = 5 shuttle run\n\n'
...
for nivel in ['Elite', 'RX', 'Intermediário', 'Scaled']:
    for num, nome, tipo, desc in PROVAS[nivel]:
        ...
        wods.append({
            ...
            'nome': nome,
            'tipo': tipo,
            'cap': CAPS[nivel].get(num) if tipo == 'tempo' else None,
            'desc': desc,
...
for nivel in PROVAS:
    ...
    caps = ' '.join('p%d=%d\u2032' % (n, c // 60) for n, c in sorted(CAPS[nivel].items()))
    print('  %-14s %d provas  %s   caps: %s' % (nivel, len(tipos), ' '.join(tipos), caps))
